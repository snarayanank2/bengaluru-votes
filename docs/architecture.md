...
`wards` (id, name_en, name_kn, corporation, boundary ref) · `candidates` (slug, ward, party, photo → media, lifecycle status `filed|contesting|rejected|withdrawn` — PRD §5.2; a new row or a status transition is the "candidate-set change" that clears ward sign-off) · `candidate_fields` (candidate, field key, value_en, value_kn, authored_lang, translation_status, source_url, source_type `official|curator`, **verification state `ai_extracted|verified|disputed|curator_confirmed`** — PRD §5.2; the published value is whatever this row holds, and the state is a label on it, never a gate in front of it) · `candidate_affidavits` (candidate, media ref, origin EC URL if fetched, extraction status; the stored copy is the public source link for affidavit fields, PRD §5.2) · `candidate_news_links` (candidate, url, title, domain, origin `auto|curator`, status `suggested|approved` — suggestions render nowhere public until a curator approves, PRD §5.2) · `media` (bytes as bytea, validated content type, sha256, size — candidate photos and affidavit PDFs, so the nightly `pg_dump` covers every curator upload with no second backup path) · `ward_issues` + per-candidate stances (deleting an issue cascades its vote-set entries; renaming keeps them — PRD §5.5) · `booths` (name_en, name_kn, address, location, ward) · `issue_votes` (user, ward, up to 3 issues; one active set per user, retired on home-ward change) · `users` (email and phone — each globally unique, either logs in, PRD §10; home ward, language, role, `src` attribution, consent record: timestamp + wording version + the optional future-civic-tools opt-in, PRD §10) · `otp_codes`, `sessions` · `suppressions` (contact, channel, reason `bounce|complaint|stop`; written by the §7 webhooks, honoured before every send) · `flags` (ward, dedupe key → count; the per-ward queue every covering curator sees, PRD §6.1) · `partners` · `eoi_submissions` · `ward_readiness` (completeness snapshot — counting only filed/contesting candidates, PRD §9.1; sign-off, cleared on candidate-set change) · `audit_log` (append-only; written in the same transaction as the change it records).

**Transcription (PRD §5.2, `docs/milestones.md` M6):** `transcription_assignments` (affidavit, transcriber, state `open|submitted|abandoned`, drawn_at, expires_at — an assignment is *held*, not owned; expiry returns the affidavit to the queue so a half-read one is never stranded) · `transcription_readings` (assignment, field key, submitted value, `confirmed_ai` flag — whether the transcriber accepted the pre-filled extraction or typed something else, which is what makes the rubber-stamp risk in PRD §5.2 measurable rather than merely acknowledged) · `field_disputes` (candidate field, the three reading ids, opened_at, resolved_by, resolved_at).

**Three invariants in this area, all enforced in the database rather than in application code:**

- **An affidavit is never assigned twice to the same transcriber.** A unique index on (affidavit, transcriber) over non-abandoned assignments. Without it, one person could satisfy the two-reading requirement alone, and the consensus guarantee is worth nothing.
- **The queue draw is atomic.** Selecting the next affidavit and creating its assignment happen in one statement (`UPDATE … RETURNING` over a randomized pick of unassigned, under-read affidavits), so two transcribers hitting *Start* in the same second cannot be handed the same document.
- **Reading counts drive state, and state changes are computed, not typed.** A field's verification state is derived from its readings when a reading lands, inside the same transaction. Nothing writes `verified` directly.

**Bulk affidavit ingestion is a capacity question, not only a code one** (`docs/milestones.md` M8). Affidavit PDFs live in `media` as `bytea`, which is deliberate — the nightly `pg_dump` then covers every upload with no second backup path — but roughly **4,000 affidavits arrive in one burst during the nomination window**, against a 20 MB per-file cap on a 16 GB box. Plausibly several GB into the database in a week, and every dump afterwards carries it. Two things to check against a real sample *before* the burst: disk headroom, and how long the nightly dump takes once it includes the affidavit set. Note also that no off-box backup destination exists yet (§10, dependency register §6.9), so a larger dump currently has nowhere to go.

Sources are per-field (PRD §11). Ward boundaries are static GeoJSON — committed at **`data/gba.geojson`** (369 wards: polygons plus ward/corporation/zone/assembly/RO metadata, EN + KN names) — served by nginx and loaded into app memory at boot for point-in-polygon lookups; a `seed-wards` script maps its feature properties into the `wards` table. The pincode → ward shortlist table that used to back a fallback lookup was removed 2026-08-14 (§11).
...
**Transcription endpoints** (PRD §5.2): `POST /api/transcribe/draw` atomically assigns the next affidavit to the calling transcriber and returns it; `POST /api/transcribe/{assignment-id}` submits a whole reading at once, recomputes the affected fields' verification states in the same transaction, opens `field_disputes` where three readings diverge, and writes the audit entries. Both are per-account rate-limited like any other authenticated write.

**These two endpoints are the platform's only scopeless privileged writes**, and they are the reason `src/middleware.ts` cannot answer authorization by asking which ward an edit targets. A transcriber has no ward scope at all (PRD §7, §10): authorization is *"does an open assignment for this affidavit belong to this caller"*, checked against `transcription_assignments`, not against `canEditWard`. The check lives with the endpoint, where the assignment id is known — the same reason per-ward curator scope is checked at the call site rather than in the middleware. The middleware's job here stops at establishing the role.

**Comparison for consensus is normalized** (PRD §5.2) and the normalization is one shared function, applied identically when a reading is stored and when readings are compared — trim, collapse whitespace, case-fold, fold Kannada digits to Latin, and parse amount and date fields to a canonical form. Two implementations of this rule that drift apart would mark agreeing readings as disputed and, worse, disagreeing readings as verified.
...
- **Cross-border processing, stated once:** Twilio, SendGrid, Google, Anthropic, and Sentry process personal data substantially outside India. DPDP §16 permits transfers absent a government restriction list, data-processing terms are executed with each vendor (dependency register §2.8), and the flows are enumerated in `/privacy` (PRD §5.16). Whatever off-box backup storage is eventually chosen must be India-resident (§10) — defense-in-depth for the copy at rest, not a claim the platform is India-resident end to end. That requirement is not yet satisfied by anything: there is no off-box copy today (§10).
...
- **Accepted limitations, recorded deliberately:**
  - The audit log is append-only at the application level only — database compromise defeats it; "immutable" (PRD §6) holds against application bugs and curator action, not against DB compromise. Nightly encrypted backups are meant to be the only historical copies outside the live database — but with no off-box backup in place (§10), there are currently **none**: today, database compromise or the box's disk failing loses the audit trail outright, not merely its off-box copy. Hash chains and off-box audit streaming were considered and rejected as infrastructure the threat model doesn't yet justify.
  ...
  - Backups are nightly only: up to 24 hours of registrations, issue votes, flags, and audit entries are lost if the box's disk dies (decided 2026-07-19). WAL archiving would shrink the window to minutes and was rejected as a second backup mechanism to operate and rehearse — even election-week write volume was judged worth less than that operational load. Recovery targets: §10. Both of these describe the intended steady state; §10 records what is actually in place as of 2026-08-13.
