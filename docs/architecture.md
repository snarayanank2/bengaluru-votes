...
- Public pages: **~60 s TTL**. `/data`: ~5 min. `/api/*`, `/account/*`, `/curator/*`, `/admin/*`: `no-store`, never cached. Issue-vote results are fetched from an uncached API only after the browser has voted.
- **Invariant: public page HTML never varies by session.** Logged-in users receive the same cached anonymous markup. Account and registration state comes from `GET /api/me`; anonymous issue-vote state comes from `GET /api/issue-votes` using an opaque HttpOnly receipt cookie. **nginx strips the `Cookie` header** before proxying public-page routes, so only the uncached API sees that receipt. Each URL has exactly one cached variant per language.
...
Issue voting uses a centrally seeded, bilingual 20-item catalog copied to every ward (`ward_issues.catalog_key`). Anonymous ballots live in `anonymous_issue_vote_sets` and `anonymous_issue_vote_selections`: the set stores only a SHA-256 hash of an opaque browser receipt, is unique across the city, and is immutable after its exactly-three selections are inserted. The older account-linked vote tables remain for migration compatibility but are not used by the citizen flow.
...
| `PUT /api/issue-votes` | Anonymous exactly-three ballot; reCAPTCHA action+score, receipt uniqueness, per-IP nginx limit, strict payload, and global daily circuit breaker |
| `GET /api/issue-votes` | Receipt status; returns ward counts and percentages only after that receipt has voted there |
| `POST /api/eoi` | Anonymous write protected by **reCAPTCHA v3** (server-verified token + action + score), disclosed in `/privacy` alongside issue voting and GA |
