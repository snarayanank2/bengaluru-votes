...
- **Polling booth lookup by EPIC number.** New, and a change of approach: the citizen gives their voter ID and the booth comes back, rather than us holding an addressed booth list and resolving to it. Tracker 127 notes an API endpoint appears to exist for this; the Maharashtra SEC ran an EPIC- and name-wise lookup at BMC 2026 (`election-timelines.md` §7). **This has not been verified against KSEC** and is the single largest unknown inside M1.
...
| **A working KSEC EPIC→booth lookup** | tracker 127, to triage |
| **The Oorvani donation URL, and the 80G / receipt wording** | tracker 94; *not listed on the sheet* |

**Risk — the EPIC path is unproven.** If KSEC exposes no such endpoint, or exposes one that cannot be called from a server, booth lookup falls back to needing an addressed polling-station list, which no precedent tells us the publication date of (`election-timelines.md` Q3) and which — since the address-based booth rows were deleted on 2026-08-15 — **nothing on the Tasks tab tracks at all.** `project-dependencies.md` §4.7 is the only place it still exists. That is a materially harder problem than the sheet's one-line description implies, and it is inside a 5-day milestone.
...
**M1 is now five separate deliverables at the same five days it carried when it was one.** Ward lookup is largely built, and the donation page is genuinely a day — but ward issues, the questions content and an unproven EPIC lookup are each their own piece of work with their own unstarted dependency. Nothing here argues for a bigger number; it argues for M1 being the milestone most likely to be declared done while a third of it is still missing. Track the five parts separately.
...
**"Find your polling booth" depends on M1's EPIC lookup being real.** Half the hard-launch message is a feature whose mechanism is untested (§3). If it does not work, the guidance degrades to ward-level — a real answer, but a materially worse one on the morning somebody is trying to find where to vote, and a weak thing to build a campaign around.
...
- **Row 127 (booth by EPIC) is now tagged M1** — it had no milestone at all despite being named scope in M1. It is still **To Triage**, and it is the single largest unknown inside M1 (§3), so triaging it is worth more than its one-line description suggests.
...
- **The address-based booth rows were deleted on 2026-08-15**, leaving row 127 (booth by EPIC) as the only booth row. That is a decision, not a tidy-up: it commits M1's booth lookup to the EPIC path, which is **unverified against KSEC** (§3). If the EPIC endpoint does not exist, the fallback those rows described — an addressed polling-station list — is no longer tracked anywhere. `project-dependencies.md` §4.7 still carries it as a dependency.
