...
- Look up ward by **address only** (no voter-ID lookup, no pincode); the input carries Google Places Autocomplete so the citizen narrows an ambiguous entry before ever submitting it; return new GBA ward name + number and corporation (N/S/E/W/Central).
...
- **Consequence: geocoding is the only path to a ward.** There is no fallback and no browsable ward list. `POST /api/ward-lookup` returns one of four result kinds (`docs/architecture.md` §11): `ward` (resolved); **out-of-coverage is an answer, not an error** — a real address outside every GBA ward returns an explicit "this doesn't appear to be in the GBA area" result, never phrased as a failure; `ambiguous` — Google matched several places or only a partial match, and the citizen can act, by being more specific; `unavailable` — our own outage, when the daily geocode budget is exhausted or Google is unreachable, phrased as "try again shortly" and never as a bad address. When lookup is `unavailable`, there is nothing else for the citizen to try.
