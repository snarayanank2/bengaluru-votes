...
|---|---|
| Ward lookup; candidate report cards (with news links) & comparison; ward issues & citizen issue-voting; registration/eligibility check; election awareness; voter-ID issuance & update; how-to-vote; booth locator; English/Kannada; registration & notifications; flag/correction workflow; transcriber, curator & admin tooling | Promise/accountability tracking; ward budgets; civic-issue officer directory; remote voting; candidate outreach tooling |

---

## 4. Users & roles

Five roles: a large, mostly read-only public, and a small, trusted group that maintains data and governs access.
...
| **Transcriber** | Invited by an admin; **no ward scope** | Read one affidavit at a time from a randomized city-wide queue; confirm or correct the AI-extracted fields (§5.2) |
| **Data curator** | Trusted, vetted individual (scoped to assigned wards, §10) | Resolve disputed fields; correct anything; define ward issue list; review flags; mark candidates complete |
| **Admin** | Small internal team | Manage roles, access, scope, users, and oversight |

**The transcriber role is new** (`docs/milestones.md` M6). It splits the job the curator role used to hold alone: transcribers read affidavits at volume, curators exercise judgement over what the readings produced. A transcriber has no ward scope by design — see §5.2.
...
- Look up ward by **typed address** (no voter-ID lookup, no pincode); the input carries Google Places Autocomplete so the citizen narrows an ambiguous entry before ever submitting it; return new GBA ward name + number and corporation (N/S/E/W/Central).
- **Or by the device's own location**, added 2026-08-14. A "use my current location" control resolves the ward from the browser's Geolocation API. It is not a variant of address lookup — the position goes straight to point-in-polygon against the ward boundaries, so it calls no external service, spends no geocode budget, and **keeps working when address lookup is `unavailable`**, which is the one state the finder previously had no answer for. Three constraints on it:
  - **The control is offered only where it works.** It is injected by client-side JS and only when the browser exposes the Geolocation API, so no-JS and unsupported visitors never see a dead button. Typed address remains the primary, always-available input.
  - **A coarse fix is qualified, not hidden.** Browsers report an accuracy radius; above 500m (typical of a desktop wifi/IP fix, where the error can exceed a ward) the resolved ward is shown *with* a visible "this is based on an approximate location" note. Withholding an answer that is usually right would serve the citizen worse than qualifying it.
  - **The position is used and dropped.** Coordinates reach the server once, resolve a ward, and are never stored, cached, or logged — the same rule §5.16 and `src/lib/geocode.ts` already hold for addresses. A refused permission is not an error state: it points back at the address field.
- **Pincode lookup was removed, 2026-08-14.** It had shipped as the Path B hedge against two risks: delimitation boundaries arriving late, and geocoding coming back ambiguous or unavailable. The boundaries landed months ago (`data/gba.geojson`), but the postal-code table itself never advanced past a 12-row placeholder — for nearly every citizen, "try your pincode" pointed at something that could not resolve, which is worse than an honest error. Places Autocomplete now covers the case the hedge was for: it makes ambiguous addresses rare at entry time, before a request ever reaches the geocoder.
- **Consequence: geocoding is the only path to a ward from a typed address.** There is no address fallback and no browsable ward list; the location control above is the only path that survives a geocoder outage, and only for a citizen who has JS, a device position, and grants permission. `POST /api/ward-lookup` returns one of four result kinds (`docs/architecture.md` §11): `ward` (resolved); **out-of-coverage is an answer, not an error** — a real address outside every GBA ward returns an explicit "this doesn't appear to be in the GBA area" result, never phrased as a failure; `ambiguous` — Google matched several places or only a partial match, and the citizen can act, by being more specific; `unavailable` — our own outage, when the daily geocode budget is exhausted or Google is unreachable, phrased as "try again shortly" and never as a bad address. When lookup is `unavailable`, there is nothing else for the citizen to try.
...
- **Affidavit ingestion is AI extraction, checked by two people.** As affidavits arrive during the nomination window, the candidate's EC affidavit (Form 26) PDF is ingested in bulk from the EC publication route, or uploaded, or its EC link pasted for the platform to fetch and store. AI extraction (the same Anthropic API dependency as machine translation — `docs/project-dependencies.md` §6.6) populates the affidavit-sourced fields above — cases, assets, education — including marking a field *not declared* where the affidavit says so (a valid, complete answer per §9.1). **Transcribers** then read the affidavit and confirm or correct each extracted value.
- **Ingestion and review are separate milestones, and separable in practice.** Acquisition, extraction and enqueuing are **M8**; the transcriber and curator surfaces are **M6** (`docs/milestones.md` §7, §9). If M6 is not ready when affidavits arrive, **M8 still publishes**: extracted fields go live marked *AI-extracted* and the enqueue step is a no-op until the queue exists. Unchecked affidavit data on a report card is worth more to a citizen than a review queue with nothing in it.
- **Every affidavit is read twice, by transcribers who did not choose it.** The queue is randomized and city-wide: a transcriber is handed the next affidavit and cannot pick a ward, a party or a candidate. This is the point of the design — two people who each chose the same ward for the same reason are not two independent readings. It is also why transcribers carry no ward scope, unlike every other privileged role (§10).
- **A field's value has a state, and the state is visible:**

  | State | Meaning | Shown as |
  |---|---|---|
  | **AI-extracted** | Extraction has run; nobody has read it | *AI-extracted* marker |
  | **Verified** | Two transcribers agreed — or two of three, after a disagreement sent it to a third reader | *Verified* marker |
  | **Disputed** | Three readings, all different. Goes to a curator; the last accepted value stays published meanwhile | *AI-extracted*, unchanged for the citizen |
  | **Curator-confirmed** | A curator confirmed or edited it | No marker |

- **Extracted fields publish immediately, mirroring the machine-translation trade (§8).** Consensus and dispute resolution happen behind an already-published value — nothing is withheld from citizens while it is being checked. The citizen flag flow (§6) remains a correction net, and extraction is audit-logged as a system entry.
- **Agreement is normalized, not literal.** Trim, collapse whitespace, case-fold, normalize Kannada and Latin digits, and compare amount and date fields as numbers and dates rather than strings — two transcribers typing `₹12,50,000` and `1250000` agree.
- **The known weakness, owned rather than solved.** Transcribers see the AI's value pre-filled and confirm or edit it. Two transcribers confirming the same wrong extraction reach consensus on it, and it is marked *verified* while being wrong. Blind entry would close this and roughly double the reading cost per affidavit. This is the same trade already made for machine-translated Kannada (§8): the nets are the curator's power to edit any field unprompted, and citizen flags.
- **Curators no longer transcribe.** They work a dispute queue, edit any field whether disputed or not, mark candidates complete, and review transcriber performance (§7).
...
- **The page ships in M1; the data is M9** (§13.1). That gap is expected to be months, not days, so the empty state is a real deliverable rather than a stopgap — it is what most visitors will see for most of the campaign. **M9 has no assumed date at all**: unlike every other milestone, nothing in the calendar estimates when the EC publishes booth data, and nobody is tracking it (`docs/project-dependencies.md` §4.7).
...
- **Held until candidate data is real** (§13.1) — not shipped with the early ward pages. Before then this page honestly reads "14 of 369 wards" — damaging, and it hands critics a number. The issue roll-up says nothing until issue voting has volume, which needs M7.
...
- **Ships with M4**, though it is really a notification-week asset — a press kit assembled at N is assembled too late, and M4 is the launch it has to serve.
...
- **`/privacy` is the earliest page on the critical path, and it gates the launch itself (M4).** Google Analytics is already live on the site, so a public launch without a published privacy policy is not an option. It also gates WhatsApp Business API onboarding — Meta requires a published privacy-policy URL — and therefore template approval and all of M7 (§9). It is not launch-week hygiene; it is the longest pole in the project.
...
### 6.4 Disputes are not flags

Two things arrive in a curator's work queue and they are not the same thing. The words are load-bearing and are used strictly, in this document and in the code.

| | **Flag** | **Dispute** |
|---|---|---|
| Raised by | A registered citizen | The system |
| Means | "This published value looks wrong" | "Three transcribers read this field and all three disagreed" (§5.2) |
| Scope | Any ward | The affidavit's ward |
| Surfaces on | `/curator/queue` | `/curator/disputes` |
| Visible to the submitter | Yes — `/account/submissions` (§6.2) | No submitter exists |

They stay separate queues. A dispute is an internal signal about data that has not been established yet; a flag is a citizen telling us something published is wrong. Collapsing them would hide the second inside the first, and the second is the one with a person waiting for an outcome.

---

## 7. Roles & permissions matrix

| Capability | Anonymous | Registered | Transcriber | Curator | Admin |
|---|:--:|:--:|:--:|:--:|:--:|
...
| **Draw the next affidavit from the queue** | – | – | **✅ City-wide** | – | – |
| **Confirm or correct an extracted field** | – | – | **✅ City-wide** | ✅ Scope | All |
| Define ward issue list | – | – | – | ✅ | ✅ |
| Mark a ward ready for candidate comms | – | – | – | Scope | All |
| **Mark a candidate complete** | – | – | – | Scope | All |
| Create / edit ward & candidate data | – | – | – | Scope | All |
| **Resolve a disputed field** | – | – | – | Scope | All |
| Publish live | – | – | – | ✅ | ✅ |
| Review submissions | – | – | – | Scope | All |
| **View transcriber performance** | – | – | – | ✅ | ✅ |
...
**"Scope" means the role's assigned wards. "City-wide" means no ward restriction at all** — and only the transcriber has it. Every other privileged capability is bounded by ward scope (`canEditWard`, `src/lib/authz.ts`); a transcriber's two capabilities are deliberately not, because the randomized queue hands them affidavits from anywhere (§5.2). That makes the transcriber the one role whose authorization cannot be answered by asking which ward the edit targets, and the enforcement path in `src/middleware.ts` has to account for it.

A transcriber can confirm or correct a field **only on an affidavit the queue has assigned to them**, and only while that assignment is open. City-wide is the absence of a ward filter, not permission to edit any candidate at will.

**Transcriber performance** — agreement rate against accepted values, volume, share of disputes contributed, median time per affidavit — is visible to curators and admins, and never to other transcribers.
...
- **One OTP mechanism for all roles.** Citizens, transcribers, curators, and admins all authenticate via lightweight **email / WhatsApp OTP** — no passwords and **no 2FA**.
- **Email OTP is the baseline; WhatsApp OTP arrives with the Business API.** Sending an OTP over WhatsApp requires completed Meta onboarding and an approved Authentication-category template (`docs/project-dependencies.md` §3), so until that path completes, login is email-OTP only.
- **Staff access degrades below even that.** An admin can generate a sign-in link for a user and deliver it by hand (`/admin/users`, M5). It exists so that admin, transcriber and curator access depends on no messaging vendor at all — if M7 is abandoned outright, the data operation still runs. At a transcriber cohort's scale, hand-issuing links daily is somebody's job rather than a workable steady state, so email OTP should be provisioned before the cohort arrives; it is a floor, not a plan.
...
- **Transcribers are the exception, deliberately.** They hold no ward scope at all, because the queue that hands them affidavits is randomized city-wide (§5.2). Their authorization is bounded by the *assignment* instead: a transcriber may write to a field only on an affidavit currently assigned to them. This is the one privileged path that cannot be authorized by asking which ward the edit targets (§7).
...
**Transcriber:** `/transcribe` · `/transcribe/{assignment-id}`

**Curator:** `/curator` · `/curator/queue` · `/curator/queue/{submission-id}` · `/curator/disputes` · `/curator/disputes/{dispute-id}` · `/curator/transcribers` · `/curator/candidate/{id}` · `/curator/ward/{id}` · `/curator/ward/{id}/issues`

**Admin:** `/admin` · `/admin/roles` · `/admin/users` · `/admin/audit` · `/admin/partners`

**Public, added by the milestone plan:** `/donate` (M2)

**Modals:** Register / Login (fallback `/login`) · Flag misinformation · Cast issue vote (top 3)

### 13.1 Shipping order

Pages do not all ship at once, because candidate data cannot exist before the EC notification (**N**). The ordering is now the milestone list — `docs/milestones.md` is authoritative; what follows is which routes each milestone opens.

| Milestone | Routes it opens |
|---|---|
| **M1** Ward discovery | `/`, `/ward/{id}`, `/ward/{id}/issues`, `/check-registration`, `/about-election`, `/voting-guide/*`, `/about` — with the candidate routes reachable and showing the pre-nomination empty state |
| **M2** Donation | `/donate` |
| **M3** Partnerships | `/partner-with-us`, `/partner/{slug}` |
| **M4** Launch | `/press`, `/privacy`, `/terms` — the legal pages gate the launch, not merely accompany it |
| **M5** Admin | `/admin/*` |
| **M6** Curator & transcriber | `/transcribe/*`, `/curator/*` |
| **M7** Registered users | `/account/*`, the `/login` fallback, and the two contribution modals; issue voting and flagging open here |
| **M8** EC affidavits available | No new routes. `/candidate/{slug}` stops being empty — affidavit-sourced fields and the stored PDF appear, and `/data` becomes meaningful |
| **M9** Booth information available | No new routes. `/voting-guide/find-booth` (live and empty since M1) starts resolving addresses to booths |

Consequences of this ordering worth stating, all changed from the earlier phase plan:

- **Issue voting no longer opens with the teaser.** It requires an account, so it follows M7 and is contingent with it (§14, `milestones.md` §11). A ward page shows its curator-defined issue list and public results as soon as they exist; the *Vote your top 3* action appears only once M7 has shipped.
- **The candidate routes are reachable from M1**, showing the pre-nomination empty state specified in IA §3.3 rather than 404ing. The URLs are shareable and will be shared early — and M4 puts launch traffic on them before any candidate exists, which is why M1 treats the empty state as a deliverable rather than a placeholder.
- **`/voting-guide/find-booth` follows the same pattern**, and for the same reason: the page ships with M1 showing an honest empty state, and **M9** is when it starts resolving addresses to booths. The launch campaign points at the voting guide, and a guide with a missing page reads worse than one with a page that says *not yet, and here is what to do meanwhile*.
- **The last two milestones open no routes at all.** M8 and M9 fill pages that already exist. That is deliberate — every URL is live and shareable from launch, and data arrives into it — but it means neither milestone is visible as a new link. Their test is whether an existing page starts answering.

`/data` is held until candidate data is real — that is, until M8 — for a different reason: it would be accurate on day one and still damaging, reading "14 of 369 wards" to anyone who looked.
...
| Decision | Resolution |
|---|---|
| Curator publish gate | Curators are trusted; edits go live immediately, no second-person approval. Unchanged by the transcriber model — consensus runs *behind* an already-published value, and "mark complete" is a coverage signal, not a visibility gate (§5.2). |
| Affidavit verification | **Two transcribers read every affidavit; a third breaks a disagreement. Two agreeing readings mark a field verified; three different readings mark it disputed and send it to a curator** (§5.2). Revised 2026-08-14 — previously one curator uploaded and spot-checked. |
| Transcriber queue | **Randomized and city-wide.** A transcriber cannot choose the ward, party or candidate they read. Consequently transcribers hold **no ward scope**, the only privileged role that does not (§7). |
| Transcriber sourcing | **Invited by an admin, not self-serve.** No public sign-up flow; the consensus mechanism is a data-quality net, not a vetting substitute. Recruitment is offline, tracked as a dependency. |
| Authentication | Single OTP mechanism for all roles (citizen, transcriber, curator, admin); no passwords, no 2FA. **Staff access does not depend on any messaging vendor:** an admin can generate a sign-in link and deliver it by hand, so admin, curator and transcriber login survives M7 being abandoned (`milestones.md` §6). |
...
| Affidavit ingestion | The EC affidavit is uploaded (file or EC link); AI extracts the affidavit fields, which publish immediately marked *AI-extracted*; transcriber consensus marks them *verified* and a curator's confirmation clears the marker; the stored PDF is the public source link (§5.2). |
| Value agreement test | **Normalized, not literal** — whitespace, case, Kannada and Latin digits normalized; amounts and dates compared as numbers and dates (§5.2). |
| Disputes vs flags | **Separate queues, separate words.** A *flag* is a citizen reporting a published error; a *dispute* is three transcribers disagreeing. Never conflated, in docs or code (§6.4). |
...
| Launch phasing | **Seven milestones replace the P0–P5 phase structure** as the plan of work (`docs/milestones.md`, 2026-08-14). Ward + logistics pages still ship first and candidate pages still fill at the EC notification; what changed is that ordering is now expressed as testable slices rather than calendar windows (§13.1). |
| Registered-user support | **Last, and abandonable.** Accounts, profiles, issue voting, flagging and the seven sends all sit in M7, behind prerequisites that are external and unowned (legal pages, Twilio, Meta verification, a `+91` number, ₹4L). If they do not land, the platform ships anonymous-only. The go-to-market plan is rewritten around that possibility rather than assuming against it (`milestones.md` §11). |
...
| Legal page sequencing | `/privacy` and `/terms` **gate M4** — the public launch cannot happen without them, since GA is already live; `/privacy` separately gates WhatsApp onboarding and therefore all of M7 (§5.16). |
...
| Success metrics | **300,000 unique visitors** this release. The **25,000 registered users** target and its ward-breadth guardrail (≥50 registrations in ≥300 of 369 wards) are **contingent on M7** — if registered-user support is abandoned, the visitor target stands alone and the registration targets are withdrawn rather than missed (§14 *Registered-user support*; `docs/gtm-plan.md`). |
...
- **Transcriber cohort (offline).** New with §5.2's verification model, and the dependency nobody has sized against people: roughly 4,000 candidates × 2 readings ≈ **8,000 transcriptions**, most of them scanned and many handwritten in Kannada, inside the window between nominations closing (~Oct 26) and report cards being due (~Nov 17). Kannada reading is a hard requirement. The software for this is three weeks (`docs/milestones.md` M6); the cohort is the binding constraint, and it does not exist.
...
- **Legal review.** `/terms` and `/privacy` need a lawyer, for DPDP Act 2023 compliance and contribution licensing. Not a product-spec deliverable, and it blocks M4 and M7 alike.
- **Press assets.** Logos, screenshots, and named spokespeople with approved quotes, for `/press` in M4.
...
- **Retention period — legal confirmation** (§5.16): the product proposal is decided — citizen contact data deleted or anonymized within **3 months of results being declared**, backups aging out on rotation. `/privacy` cannot ship until the lawyer confirms (or amends) the period, so this stays the blocker at the head of the legal chain.
...
- **Is Citizen Matters an owned channel?** Likely — Oorvani publishes Citizen Matters alongside Open City — but unconfirmed. If confirmed, the GTM plan's cold-start assumption is wrong in the project's favour and launch distribution should be re-planned around it — which matters more now that reach carries the programme alone (`docs/gtm-plan.md` §1). *(= `docs/project-dependencies.md` 7.4)*
