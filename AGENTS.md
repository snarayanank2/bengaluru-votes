...
**Candidate-data operation narrative:** Present the candidate-data document and deck in operational order: bulk loading by Siva (assuming all election affidavits are available online), AI extraction of candidate fields with one confidence percentage per affidavit, transcriber review, and user-flag resolution by curators. The transcriber queue serves affidavits with missing data first, low-confidence affidavits next, and the rest afterwards. Curators respond to user-flagged candidate-data problems only: a flag enters their queue, and the curator either dismisses it as incorrect or fixes the data. Do not assign routine transcription oversight or unflagged cases to curators in this operation. Keep both the AI elapsed-time budget and API cost budget beside the extraction stage, and distinguish planning assumptions from measured results.

**Start candidate-data processing on initial availability:** Siva begins loading as soon as initial affidavit data is available online; do not wait for a final notification, final candidate list or complete bulk set. AI extraction and transcription can proceed on ready records while loading continues. Reconcile rejected/withdrawn candidates against later official lists. Present the three-day AI and five-day transcription budgets separately from the ongoing user-flag curation rota through election day; distinguish a processing allowance from a guaranteed finish date when source data arrives in stages.
